...
let pp_scalar fmt s =
  match s with
  (* These are all mapped to OCaml's 'int' in your driver *)
  ...
  | Sc.IsInt32 v   -> Format.fprintf fmt "%ldl" v
  | Sc.IsUInt32 v  -> Format.fprintf fmt "%ldl" v (* Mapped to int32 in your driver *)

  (* These are mapped to OCaml's 'int64' *)
  | Sc.IsInt64 v   -> Format.fprintf fmt "%LdL" v
  | Sc.IsUInt64 v  -> Format.fprintf fmt "%LdL" v (* Mapped to int64 in your driver *)

  (* These are mapped to OCaml's 'float' *)
  | Sc.IsFloat32 v -> Format.fprintf fmt "%.7f" v
...
let add = Opadd__Add.add_tensors