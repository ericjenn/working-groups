...
SB: As discussed yesterday, it is fine to exclude broadcasting, but the argument against it here is not correct. 
Broadcasting can occur in neural networks without any dynamic tensor shapes.
For `where` a natural usecase would be `X` being any tensor and `Y` containing just a single scalar value.
### Signature
...
SB: non-zero can be dangerous, if `condition` is a float tensor. Such a network could easily become unstable due to floating point inaccuracies. 
Maybe we need tighter restictions on `condition`, i.e., enforce it to be integral. 
