...
struct __coords_from_X_p_result coords_from_x_p(struct ctensor_ui8 x,
  ...
  if (x_coords_array) {
    ...
    if (x_coords >= 0) {
      result.__field_0 = ((int32_t) (x.t_data[x_coords])) - ((int32_t) x_zero);
      result.__field_1 = flag;
      return result;
    } else {
      result1.__field_0 = 0 - ((int32_t) x_zero);
...
struct __w_cools_calculate_result w_cools_calculate(struct ctensor_ui8 x,
  ...
  int32_t cols, jj, o, x_h, x_w, x_val;
  int aux_flag;
  struct __coords_from_X_p_result struct_res;
  int32_t w_coords, w_val;
  int8_t w_raw;
  ...
  if (x_coords_array && w_coords_array) {
    ...
    if (0 <= o) {
      for (jj = 0; ; ++jj) {
        ...
        struct_res = coords_from_x_p(x, x_zero, x_coords_array, pad_top,
                     pad_left);
        ...
        if (aux_flag) {
          ...
          w_raw = w.t_data[w_coords];
          w_val = ((int32_t) w_raw) - ((int32_t) w_zero);
          sum = (int32_t)((uint32_t)sum + (uint32_t)(int32_t)((uint32_t)x_val * (uint32_t)w_val));
        } else {
          flag = 0;
          result.__field_0 = (((int32_t) 0));
          result.__field_1 = flag;
          return result;
        }
        if (jj == o) {
...
struct __w_lines_calculate_result w_lines_calculate(struct ctensor_ui8 x,
  ...
  if (0 <= o) {
    for (ii = 0; ; ++ii) {
      struct_res = w_cools_calculate(x, w, x_zero, w_zero, c, ii, n, m, y_h,
                   y_w, str_h, str_w, dil_h, dil_w, pad_top, pad_left);
      value = struct_res.__field_0;
      aux_flag = struct_res.__field_1;
      if (aux_flag) {
        flag = 1;
        sum = (int32_t)((uint32_t)sum + (uint32_t)value);
      } else {
        flag = 0;
        result.__field_0 = (((int32_t) 0));
        result.__field_1 = flag;
...
struct __w_channels_calculate_result w_channels_calculate(struct ctensor_ui8 x,
  ...
  if (0 <= o) {
    for (cc = 0; ; ++cc) {
      struct_res = w_lines_calculate(x, w, x_zero, w_zero, cc, n, m, y_h,
                   y_w, str_h, str_w, dil_h, dil_w, pad_top, pad_left);
      value = struct_res.__field_0;
      aux_flag = struct_res.__field_1;
      if (aux_flag) {
...
int cconvinteger(struct ctensor_ui8 x, struct ctensor_i8 w, uint8_t x_zero,
                 ...
                 int32_t pad_right) {
  int32_t n, m, y_h, y_w, nn, o, mm, o1, y_hh, o2, y_ww, o3, value;
              ...
              for (y_hh = 0; ; ++y_hh) {
                ...
                if (0 <= o3) {
                  for (y_ww = 0; ; ++y_ww) {
                    ...
                    struct_res = w_channels_calculate(x, w, x_zero, w_zero,
                                 nn, mm, y_hh, y_ww, str_h, str_w, dil_h,
                                 dil_w, pad_top, pad_left);
                    value = struct_res.__field_0;
